...
export function useViajeResourceOptions(enabled: boolean = true) {
    const { data: clientes } = useQuery({
        ...
        queryFn: async () => (await clienteApi.getSelect()).data ?? [],
        enabled,
    });

    const { data: tractos } = useQuery({
        queryKey: VIAJE_QUERY_KEYS.options.tractos(),
        queryFn: async () => (await flotaApi.getSelectTipo(TIPO_FLOTA_CODES.CAMIONES, 50)).data ?? [],
        enabled,
    });

    const { data: carretas } = useQuery({
        queryKey: VIAJE_QUERY_KEYS.options.carretas(),
        queryFn: async () => (await flotaApi.getSelectTipo(TIPO_FLOTA_CODES.CARRETAS, 50)).data ?? [],
        enabled,
    });

    const { data: flotasEscolta } = useQuery({
        queryKey: VIAJE_QUERY_KEYS.options.flotasEscolta(),
        queryFn: async () => (await flotaApi.getSelect(undefined, 100)).data ?? [],
        enabled,
    });

    const { data: colaboradores } = useQuery({
        queryKey: VIAJE_QUERY_KEYS.options.colaboradores(),
        queryFn: async () => (await colaboradorApi.getSelect()).data ?? [],
        enabled,
    });
