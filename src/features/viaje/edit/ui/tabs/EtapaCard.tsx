...
export function EtapaCard({ etapa, viajeId, isViewOnly }: EtapaCardProps) {
    ...
    const principal = etapa.items.find((item) => item.esOpcionPrincipal) || etapa.items[0];
    const alternativas = etapa.items.filter((item) => item.viajeControlRutaId !== principal?.viajeControlRutaId);
    ...
    return (
        <>
            ...
            <EditRutaDialog open={!!editingRuta} onClose={() => setEditingRuta(null)} viajeId={viajeId} ruta={editingRuta} />
