...
export function ViajeIncidenteList({ viajeId, viewOnly, tiposIncidente, onEdit }: Props) {
    ...
    const handleEditClick = () => {
        if (selectedId && onEdit) {
            const item = incidentes.find(i => i.viajeIncidenteID === selectedId);
            if (item) onEdit(item);
        }
        handleMenuClose();
    };
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, height: '100%' }}>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2, pr: 1 }}>
                {isLoading ? (
                    ...
                    incidentes.map((item) => {
                        const tipoText = tiposIncidente.find(t => t.id === item.tipoIncidenteID)?.text || item.tipoIncidente?.descripcion || 'Otro';
                                ...
                                <Box sx={{ p: 3, flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        ...
                                        {!viewOnly && (
                                            <IconButton 
                                                size="small" 
                                                onClick={(e) => handleMenuOpen(e, item.viajeIncidenteID)}
